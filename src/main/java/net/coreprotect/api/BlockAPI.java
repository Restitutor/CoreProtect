...
public class BlockAPI {
     ...
     */
    private static PreparedStatement createLocationQuery(Connection connection, String tableType, String selectClause, Block block, int checkTime) throws Exception {
        int x = block.getX();
        int y = block.getY();
        int z = block.getZ();
    ...
    public static List<String[]> performLookup(Block block, int offset) {
        ...
        try (Connection connection = Database.getConnection(false, 1000)) {
            ...
            try (PreparedStatement statement = createLocationQuery(connection, "block", "time,user,action,type,data,blockdata,rolled_back", block, checkTime)) {
    ...
    /**
     * Performs a lookup of container-related actions at the specified block.
     *
     * @param block  The block to look up
     * @param offset Time constraint in seconds (0 means no time constraint)
     * @return List of results in a String array format
     */
    public static List<ContainerResult> performContainerLookup(Block block, int offset) {
        ...
        try (Connection connection = Database.getConnection(false, 1000)) {
            ...
            try (PreparedStatement statement = createLocationQuery(connection, "container", "time,user,type,data,amount,metadata,action,rolled_back", block, checkTime)) {
                try (ResultSet results = statement.executeQuery()) {
                    while (results.next()) {
                        int resultUserId = results.getInt("user");
                        ...
                        ContainerResult containerResult = new ContainerResult(
                                resultTime, resultUser, block.getWorld().getName(), x, y, z,
